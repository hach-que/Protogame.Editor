...
namespace Protogame.Editor.Api.Version1.Layout
{
    ...
    public class DockableLayoutContainer : IDockableContainer, ITabbableContainer
    {
        ...
        {
            var regionSize = layout;
            var tabs = new List<TabForRendering>();
        ...
        public void RemoveInnerRegion(ITabbableContainer child)
        {
            if (_innerRegions.Contains(child))
            {
                child.Parent = null;
                _innerRegions.Remove(child);
            }
        }
        ...
        {
            if (ActiveTabIndex != -1 && (ActiveTabIndex >= _innerRegions.Count || !_innerRegions[ActiveTabIndex].Visible))
            {
                // Switch to a visible tab.
                var didFindVisible = false;
                for (var i = ActiveTabIndex; i >= 0; i--)
                {
                    if (ActiveTabIndex < _innerRegions.Count && _innerRegions[i].Visible)
                    {
                        ActiveTabIndex = i;
                        didFindVisible = true;
                    }
                ...
                if (!didFindVisible)
                {
                    ...
                    {
                        if (ActiveTabIndex >= 0 && _innerRegions[i].Visible)
                        {
                            ActiveTabIndex = i;
                            didFindVisible = true;
                        }
